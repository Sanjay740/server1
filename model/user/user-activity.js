...
exports.getAllTasks = function (req) {
    return new Promise((resolve, reject) => {
        ...
        if (!!req.params.id) {
            let getQuery = `SELECT task.id,task.task_name,task.type, task.description,task.priority,task.is_delayed, task.created_date, 
         ...
         task.user_id = user.id LEFT JOIN category ON task.category_id = category.id WHERE task.assigned_by='${req.params.id}' ORDER BY task.id DESC;`
            db.query(getQuery, function (err, manageTask) {
                if (err) {
                    ...
                    let myTaskQuery =`SELECT c.category_name,t.id,t.task_name,t.type,
                    t.description,t.priority,t.is_delayed, t.created_date, 
                    t.completion_date,t.completed_on,t.status,
                    u.full_name,u.email,tp.answer from task t
                    join category c on t.category_id = c.id
                    join user u on u.id=t.assigned_by
                    left join task_polling tp on t.id = tp.task_id
                    and tp.user_id = ${req.params.id}
                    where t.user_id in (${req.params.id},0) AND t.assigned_by <> '${req.params.id}' ORDER BY t.id DESC;`  
                    
                    // let myTaskQuery = `SELECT task.id,task.task_name, task.description,task.priority,task.is_delayed, task.created_date,
                    // task_polling.answer, task.completion_date,task.completed_on,task.status,task.type, user.full_name,user.email,task.user_id,category.category_name
                    // FROM task LEFT JOIN user ON
                    // task.assigned_by = user.id LEFT JOIN category ON task.category_id = category.id
                    // LEFT JOIN task_polling ON ( task.user_id = task_polling.user_id OR  task.user_id = 0) AND task.id = task_polling.task_id
                    // WHERE task.user_id='${req.params.id}' OR (task.user_id= '0' AND task.assigned_by <> '${req.params.id}') ORDER BY task.id DESC;`
...
exports.updatePollStatus = function (req) {
    return new Promise((resolve, reject) => {
        ...
        if(!!req.body)
        {
        let InsertQuery = `INSERT INTO task_polling(task_id, user_id, answer,remark) VALUES('${req.body.id}','${req.body.user_id}','${req.body.answer}','${req.body.remark}');`
            db.query(InsertQuery, function (err, result) {
            if (!!result) {
                let obj = {
                    success: true,
